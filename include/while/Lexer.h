...
namespace while_lang {
	...
	//list of new tokens for while
	std::vector<std::pair<std::string, Token>> tok_map{
		{";", Token::tok_seperate},
		{":=", Token::tok_asign},
		{"if", Token::tok_if},
		{"then", Token::tok_then},
		...
		Token getTok() {
			...
			if (isalpha(lastChar)) {
				...
				if (identifierStr == "var")
					return tok_var;
				//scan for while language specific tokens
				for (const auto& el : tok_map) {
					if (identifierStr == el.first) {
						return el.second;
					}
				}
